...
class Section4(db.Model):
    ...
    task = db.relationship('Task4',cascade="all, delete, delete-orphan", lazy=True )  

    projects = db.relationship("Project4", backref='project4_sections', cascade="all, delete", lazy=True)

    def to_dict(self):
        tasks = {task.id:task.to_dict() for task in self.section4_tasks}    
        return{
            'id': self.id,
